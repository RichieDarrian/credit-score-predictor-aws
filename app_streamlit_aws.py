"""
Streamlit UI for Credit Score classifier hosted on SageMaker.
"""
import json
import os
import boto3
...
# Helpers
def build_instance(inputs: dict) -> list:
    """Return a single flat list matching FEATURE_NAMES order."""
    loan_flags = {col: int(col in inputs["loan_types"]) for col in LOAN_COLS}
    row = {
...
def invoke_endpoint(inputs: dict) -> dict:
    runtime = get_runtime_client()
    payload = {"instances": [build_instance(inputs)]}
...
if submitted:
    ...
    try:
        ...
        prediction_code = result["predictions"][0]
        prediction_label = LABEL_MAP.get(prediction_code, result["labels"][0])
